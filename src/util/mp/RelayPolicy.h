...
restore_collective( void ) {
  ...
  if( size!=world_size )
    ERROR(( "The number of processes that made this checkpt (%i) is different "
            "from the number of processes currently (%i)", size, world_size ));
  if( rank!=world_rank )
    ERROR(( "This process (%i) is reading a checkpoint previously written by "
            "a different process (%i)", rank, world_rank ));
  return world;
}
