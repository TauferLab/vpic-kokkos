...
void vpic_simulation::write_fields_hdf5(DumpParameters& params, field_array_t* fa, bool single_file) {
    KOKKOS_TIC() {
        ...
        // Define Variable Map & Identify Active Variables
        ...
        // ---------------------------------------------------------------------
        // Phase 3: Buffer Packing (AoS to SoA repack)
        // ---------------------------------------------------------------------
        size_t num_cells = g->nx * g->ny * g->nz;
        ...
        // t_compute unused for fields (no equivalent of center_p_dump)
        t_compute = 0.0;

        // ---------------------------------------------------------------------
        // Phase 4: Pure I/O (H5Dwrite)
        // ---------------------------------------------------------------------
        t_phase_start = wallclock();
...
void vpic_simulation::write_hydro_hdf5(DumpParameters& params, hydro_array_t* ha, const char* sp_name, bool single_file) {
    KOKKOS_TIC() {
        ...
        // t_compute: hydro accumulation is in setup since it must precede everything
        t_compute = 0.0;
