...
/**
 * @brief Writes particle data to binary format with time-centering
 * 
 * @param fbase Base filename (without extension or rank suffix)
 * @param species_name Name of species to dump
 * 
 * This function performs several operations before writing particle data:
 * 1. Time-centering: Particles are centered to the current timestep via center_p_dump()
 * 2. Chunked I/O: Large particle arrays are processed in PARTICLE_BUFFER_SIZE chunks
 *    to limit memory usage
 * 3. Ghost filtering: Only physical particles are written (ghosts removed by center_p_dump)
 * 
 * Each particle contains 8 floats: (dx, dy, dz, ux, uy, uz, w) + voxel index i
 * Data is written in rank-per-file (M2M) format.
 * 
 * @note Time-centering is performed during I/O by design - particles must be
 *       centered for post-processing analysis. The chunked approach allows this
 *       to happen without allocating a full copy of the particle array.
 */
void vpic_simulation::write_particles_binary(const char* fbase, const char* species_name) {
    TIC {
        ...
        // Ensure particle data is available on host
        sp->copy_to_host();
        ...
        // WRITE HEADER
        // =====================================================================
        // Header contains grid metadata and particle count.
        // num_vars=8 represents the 8 components per particle:
        //   Position: dx, dy, dz (cell-relative coordinates)
        //   Momentum: ux, uy, uz (4-velocity * gamma)
        //   Weight: w (macroparticle weight)
        //   Voxel: i (encoded cell index)
        ...
        BinaryHeader header = {
            ...
            g->dt,
            g->dx, g->dy, g->dz,
            g->x0, g->y0, g->z0,     // Global origin (used for particles)
            sp->q / sp->m,            // Charge-to-mass ratio
            8,                        // Number of components per particle
            (int32_t)my_count         // Particle count (stored in var_mask field)
        };
        fwrite(&header, sizeof(BinaryHeader), 1, fp);

        // =====================================================================
        // CHUNKED PARTICLE PROCESSING
        // =====================================================================
        // Process particles in fixed-size chunks to:
        // 1. Limit memory usage (no full array copy needed)
        // 2. Allow time-centering to happen incrementally
        // 3. Enable streaming to disk as chunks complete
        
        particle_t* p_buf;
        MALLOC_ALIGNED(p_buf, PARTICLE_BUFFER_SIZE, 128);

        // Cache host views and species state
        auto& k_p_h = sp->k_p_h;
        ...
        for (int buf_start = 0; buf_start < sp_np; buf_start += PARTICLE_BUFFER_SIZE) {
            
            // -------------------------------------------------------------
            // CHUNK EXTRACTION
            // -------------------------------------------------------------
            // Temporarily spoof species counts to work with current chunk
            
            sp->np = sp_np - buf_start;
            if (sp->np > PARTICLE_BUFFER_SIZE) sp->np = PARTICLE_BUFFER_SIZE;
            sp->max_np = PARTICLE_BUFFER_SIZE;

            // Copy chunk from Kokkos views to aligned buffer
            Kokkos::View<particle_t*, Kokkos::HostSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged>> 
                pbuf_view(p_buf, PARTICLE_BUFFER_SIZE);
            ...
            // -------------------------------------------------------------
            // TIME-CENTERING & GHOST REMOVAL
            // -------------------------------------------------------------
            // center_p_dump performs:
            // 1. Boris push to synchronize particle positions/momenta to current time
            // 2. Ghost particle removal (updates sp->np to valid count)
            // This is done in-place on p_buf.
            ...
            // WRITE CHUNK
            // -------------------------------------------------------------
            // Only write valid particles (ghost count may have reduced sp->np)
        ...
        // Restore original species state
        sp->np = sp_np;
        sp->max_np = sp_max_np;
...
/**
 * @brief Writes particle data to HDF5 format with time-centering
 * 
 ...
 * 
 * This function performs chunked I/O with time-centering similar to binary writer:
 * 1. Particles are processed in PARTICLE_BUFFER_SIZE chunks to limit memory
 * 2. Each chunk is time-centered via center_p_dump() before writing
 * 3. Ghost particles are filtered out during centering
 * 
 * Outputs two metrics:
...
void vpic_simulation::write_particles_hdf5(const char* fbase, const char* species_name, bool single_file) {
    KOKKOS_TIC() {
        ...
        double t_compute = 0.0;      // Time-centering physics
        double t_h5dwrite = 0.0;
        double t_meta_close = 0.0;

        // =====================================================================
        // PHASE 1: SETUP & OFFSET CALCULATION
        // =====================================================================
        ...
        #ifdef VPIC_HDF5_PARALLEL
        if (single_file) {
            // Compute global particle count and this rank's offset via MPI scan
        ...
        // Create file with optimized FAPL
        ...
        // Define HDF5 compound datatype matching particle_t memory layout
        ...
        // =====================================================================
        // PHASE 3: CHUNKED PROCESSING PIPELINE
        // =====================================================================
        // Process particles in fixed-size chunks to enable streaming I/O
        
        const int PBUF_SIZE = 2097152; // 2M particles ≈ 64MB per chunk
        particle_t* p_buf;
        MALLOC_ALIGNED(p_buf, PBUF_SIZE, 128);

        // Cache views and species state
        auto& k_p_h = sp->k_p_h;
        auto& k_p_i_h = sp->k_p_i_h;
        int sp_np = sp->np;
        int sp_max_np = sp->max_np;

        // Calculate total loop iterations (synchronized across ranks for M2O)
        ...
        for (int loop_idx = 0; loop_idx < total_loops; ++loop_idx) {
            ...
            if (buf_start < sp_np) {
                // Temporarily adjust species counts for current chunk
                sp->np = sp_np - buf_start;
                if (sp->np > PBUF_SIZE) sp->np = PBUF_SIZE;
                sp->max_np = PBUF_SIZE;

                // ---------------------------------------------------------
                // Sub-phase 3a: Buffer Packing
                // ---------------------------------------------------------
                
                double t_loop_start = wallclock();
                
                Kokkos::View<particle_t*, Kokkos::HostSpace, 
                ...
                center_p_dump(sp, p_buf, interpolator_array);
                chunk_valid_count = sp->np;  // Updated by center_p_dump
                t_compute += (wallclock() - t_loop_start);
            }

            // -------------------------------------------------------------
            // Sub-phase 3c: Write Chunk
            // -------------------------------------------------------------
            // M2O: All ranks must participate (collective I/O)
            // M2M: Skip if no valid particles in this chunk
        ...
        double mb_total = 0.0;
        if (single_file) {
            mb_total = (double)(total_count * sizeof(particle_t)) / (1024.0 * 1024.0);
        } else {
            double mb_local = (double)(my_count * sizeof(particle_t)) / (1024.0 * 1024.0);
            mp_allsum_d(&mb_local, &mb_total, 1);
        }
